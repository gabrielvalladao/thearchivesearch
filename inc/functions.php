...
function headerElem(){ // header() is already a function in PHP
 ...
 <a class='logo' href='".HOST."'><strong><img class='logo-front' alt='Spaceheel' height='30' src='".HOST."/cdn/img/logo-front.png' width='auto' style='margin-top: 5px;'></strong></a> 
 <form method='GET' action='".HOST."/search.php' class='searchForm'>
 <input id='query' type='text' placeholder='Busque informações relevantes' autocomplete='off' name='q' value=\"".$GLOBALS['displayQ']."\"/><button><svg viewBox='0 0 100 100' class='shape-search'><use xlink:href='#shape-search'></use></svg></button>
 </form>
 </div>  
...
function footer(){
 $footer = "<div class='footer'><a href='".HOST."/admin/index.php'>Admin</a><a href='".HOST."/about/about.php'>About</a><a href='".HOST."/about/stats.php'>Stats</a><a href='".HOST."/about/bot.php'>Dingo</a><div style='float:right;'>&copy; Copyright <a target='_blank' href='http://gabrielvalladao.com'>Gabriel Valladão</a>".date("Y")."</div></div>";
 $footer.='
...
function getResults(){
 ...
 if($q!=null){
  ...
  $sql=$GLOBALS['dbh']->prepare("SELECT `title`, `url`, `description`, `icons` FROM search WHERE `title` LIKE :q OR `url` LIKE :q OR `description` LIKE :q ORDER BY id");
  $sql->bindValue(":q", "%$q%");
  $sql->execute();
  ...
  if($sql->rowCount()==0 || $start>$sql->rowCount()){
   ...
   foreach($limitedResults as $r){
    $res["results"][]=array($r['title'], $r['url'], $r['description'], $r['icons']);
   }
   return $res;
  }
 }
